...
public class Board
{
  ...
  public string Print()
  {
    ...
    for (var j = 0; j < ColumnCount; j++) bldr.Append(j.ToString("00"));
  ...
  public void PrintToConsole()
  {
    ...
    for (var j = 0; j < ColumnCount; j++) System.Console.Write(j.ToString("00"));
  ...
  private void GenerateBoard()
  {
    ...
    for (var i = 0; i < Words.Count; i++)
    {
      ...
      if (!locations.Any()) throw new Exception("no locations");
    ...
    for (var j = 0; j < ColumnCount; j++)
    {
      if (Matrix[i, j].Type != Cell.CellType.Empty) continue;

      if (messageChars.Count == 0) break;
